...
(define (query-driver-loop)
  ...
  (let ([q (query-syntax-process (read))])
    (cond [(assertion-to-be-added? q)
           ...
           (newline)
           (display "Assertion added to date base.")
           (query-driver-loop)]
          [#t
           (newline)
...
(deifne (always-true ignore frame-stream) frame-stream)
...
(define (stream-append-delayed s1 delayed-s2)
  (if (stream-null? s1)
      ...
      (cons-stream
       (stream-car s1)
       (stream-append-delyed (stream-cdr s1)
                             delayed-s2))))
...
(define (rest-conjucnts exps) (cdr exps))
