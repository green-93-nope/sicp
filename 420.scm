(define (letrec->let exp)
  (let ((parameters (cadr exp))
        (body (cddr exp)))
    (let ((vars (map car parameters))
          (exps (map cadr parameters)))
...
(define (let->combination exp)
  (if (or (pair? (cadr exp)) (null? (cadr exp)))
...
(define (scan-out-defines procedure-body)
  (define (is-define? bodys)
    (and (pair? (car bodys)) (eq? (caar bodys) 'define)))
  (define (get-all-define-bodys bodys)
    (cond ((null? bodys) '())
          ((is-define? bodys) (cons (cdar bodys) (get-all-define-bodys (cdr bodys))))
          (else (get-all-define-bodys (cdr bodys)))))
  (define (get-other-bodys bodys)
    (cond ((null? bodys) '())
          ((is-define? bodys) (get-other-bodys (cdr bodys)))
          (else
           (cons (car bodys)
                      (get-other-bodys (cdr bodys))))))
  (let ((define-bodys (get-all-define-bodys procedure-body)))
    (if (null? define-bodys)
        procedure-body
        (let ((define-vars (map car define-bodys))
              (define-vals (map cadr define-bodys))
              (last-bodys (get-other-bodys procedure-body)))
          (cons 'let (cons
      ...
      (cond ((null? vars)
            ...
            ((eq? var (car vars))
             (if (eq? (car vals) '*unassigned*)
                 (error "This variable is unassigned" var)
                 (car vals)))
